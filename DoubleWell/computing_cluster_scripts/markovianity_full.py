...
def compute_fpt_dist(labels,delay,kd_max=5000,tol=1e-10,dt=.05):
...
def main(argv):
    ...
    parser.add_argument('-delayf','--Df',help="last delay",default=2000,type=int)
    ...
    t0_ = time.time()
    #path for split symbolic sequences
    ...
    f = h5py.File(outpath+'kinetic_properties_k_B_T_{}_clusters_{}_idx_{}_delays_{}_{}.h5'.format(k_B_T,n_clusters,args.Idx,d0,df),'w')
    ...
    for kd,delay in enumerate(delay_range):
        try:
            ...
            print('Could not compute for {} delays'.format(delay))
            continue
        #print(kd,flush=True)
